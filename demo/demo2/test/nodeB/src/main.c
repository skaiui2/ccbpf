...
#define HOOK_SOCK_PATH "/tmp/ccbpf_hook_nodeB.sock"
...
int main(void)
{
    ...
    if (hook_control_init() < 0) {
        fprintf(stderr, "[B] hook_control_init failed\n");
        return 1;
    }
    ...
    if (access("testB.bin", F_OK) != 0) {
        ...
        if (gen < 0) {
            perror("open testB.bin for write");
            return 1;
        }
        for (size_t i = 0; i < TEST_FILE_SIZE; i++) {
            uint8_t b = i % 256;
            if (write(gen, &b, 1) != 1) {
                perror("write testB.bin");
                close(gen);
                return 1;
            }
        }
    ...
    cal_udp_ctx_t udp;
    if (cal_udp_open(&udp, "127.0.0.1", 6000) != 0) {
        fprintf(stderr, "[B] cal_udp_open failed\n");
        close(fd_send);
        close(fd_recv);
        return 1;
    }
